...
private:
	...
	// step4: Render items divided by PSO.
	//std::vector<RenderItem*> mOpaqueRitems;
	std::vector<RenderItem*> mRitemLayer[(int)RenderLayer::Count];

	// If we  use Waves class to draw the waves, we would use a triangle grid mesh like we did with the peaks and valleys, 
...
bool WavesApp::Initialize()
{
	...
	mWaves = std::make_unique<Waves>(128, 128, 1.0f, 0.03f, 4.0f, 0.02f);
...
void WavesApp::Update(const GameTimer& gt)
{
	...
	//step4: Because this wave function also depends on time t (i.e., the wave surface changes with time), 
